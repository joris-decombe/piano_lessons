...
export const Controls = memo(function Controls({
    ...
    return (
        <div className="relative w-full">
            ...
            <div className={`relative w-full pixel-panel px-4 py-2 flex items-center justify-between gap-4 ${isTouch ? 'h-[72px] md:h-[80px]' : 'h-[56px] md:h-[64px]'}`}>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={`flex-shrink-0 flex items-center justify-center pixel-btn ${isTouch ? 'w-10 h-10' : 'w-8 h-8 md:w-10 md:h-10'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-shrink-0 flex items-center justify-center pixel-btn-primary ${isTouch ? 'w-14 h-14' : 'w-10 h-10 md:w-12 md:h-12'}`}
                    >
                        {isPlaying ? (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`${isTouch ? 'w-6 h-6' : 'w-5 h-5'}`}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25v13.5m-7.5-13.5v13.5" />
                            </svg>
                        ) : (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`${isTouch ? 'w-6 h-6' : 'w-5 h-5 ml-0.5'}`}>
            ...
            {/* Settings Popover - Pixel Art Style */}
            {isSettingsOpen && (
                <>
                    <div className="fixed inset-0 z-40" onClick={() => setIsSettingsOpen(false)} />
                    <div className="absolute bottom-full right-0 mb-4 w-[280px] pixel-panel p-3 z-50 flex flex-col gap-3">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className="w-full flex items-center justify-between pixel-inset p-2 text-sm text-[var(--color-text)] hover:text-[var(--color-text-bright)]"
                                    >
                                        <span className="truncate">{songSettings.currentSong.title}</span>
                                        <svg className={`w-4 h-4 text-[var(--color-subtle)] ${isSongMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                                    </button>
                                    {isSongMenuOpen && (
                                        <div className="absolute bottom-full left-0 mb-2 w-full pixel-panel overflow-hidden max-h-[200px] overflow-y-auto">
                                            {songSettings.songs.map(song => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-3 py-2 text-xs ${song.id === songSettings.currentSong.id ? "bg-[var(--color-ui-active)] text-[var(--color-text-bright)]" : "text-[var(--color-subtle)] hover:bg-[var(--color-elevated)] hover:text-[var(--color-text)]"}`}
                                                >
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="pixel-inset p-1">
                                <input
                                    ...
                                    className={`w-full cursor-pointer bg-transparent appearance-none accent-[var(--color-accent-primary)] touch-none ${isTouch ? 'h-4' : 'h-2'}`}
                                />
                            </div>
                        </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="pixel-inset p-2 space-y-2">
                                        ...
                                        <div className="flex gap-1">
                                            <button
                                                className={`flex-1 text-xs py-1 ${visualSettings.splitStrategy === 'tracks' ? 'pixel-btn-primary' : 'pixel-btn'}`}
                                                onClick={() => visualSettings.setSplitStrategy('tracks')}
                                            >
                                                Tracks
                                            </button>
                                            <button
                                                className={`flex-1 text-xs py-1 ${visualSettings.splitStrategy === 'point' ? 'pixel-btn-primary' : 'pixel-btn'}`}
                                        ...
                                        {visualSettings.splitStrategy === 'point' && (
                                            <div className="pt-1 space-y-1">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full h-2 bg-transparent appearance-none accent-[var(--color-accent-primary)]"
                                                />
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <div className="flex items-center gap-2 pixel-inset p-2">
                                            <div className="w-4 h-4 border-2 border-[var(--color-border)] overflow-hidden flex-shrink-0">
